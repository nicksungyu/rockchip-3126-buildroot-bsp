				...
				# xxx_defconfig
					...
					if [ ! -f configs/${ARG_BOARD}_defconfig -a ! -f configs/${ARG_BOARD}.config ]; then
						...
						if [ $PACK_DEBUG == "release" ]; then
							#echo "==================uboot release============"
							sed -i 's/CONFIG_BOOTDELAY=0/CONFIG_BOOTDELAY=-2/g' .config
							sed -i '$aCONFIG_SILENT_U_BOOT_ONLY=y' .config
							sed -i 's/\# CONFIG_DISABLE_CONSOLE is not set/CONFIG_DISABLE_CONSOLE=y/g' .config
							sed -i 's/\# CONFIG_SILENT_CONSOLE is not set/CONFIG_SILENT_CONSOLE=y/g' .config
							sed -i '$aCONFIG_SILENT_CONSOLE_UPDATE_ON_SET=y' .config
							sed -i '$aCONFIG_SILENT_CONSOLE_UPDATE_ON_RELOC=n' .config
						#elif [ $PACK_DEBUG == "debug" ]; then
							#echo "==================uboot   debug============"
						fi
					fi
				fi
