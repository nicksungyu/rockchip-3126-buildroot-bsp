...
int rk_recoverytxt_exist(void){
	char*  recoverytxt_argv[6] = { "fatload", "rknand", "0:7", "0x00000000", "recovery.txt", NULL };

	if(do_fat_fsload(0, 0, 5, recoverytxt_argv)){
		printf("rk3126 recoverytxt info error : unable to open recovery file %s\n", recoverytxt_argv[4]);
		return 1;
	} // if
	
	else printf("rk3126 open successfully recovery.txt\n");
...
fallback:
	...
	ret = rk_recoverytxt_exist();
	if(!ret){
		printf("recovery.txt found, bte recovery\n");
		boot_mode = BOOT_BTE_RECOVERY;

		snprintf(boot_options, sizeof(boot_options),"androidboot.mode=bte_recovery ");
