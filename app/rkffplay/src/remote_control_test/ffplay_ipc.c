...
static void* run_ffplay_thread(void *arg) {
    ...
    if (ipc_state.start_time > 0) {
        snprintf(cmd, sizeof(cmd), "/usr/bin/rkffplay -hide_banner -nostats -an -fs -ss %d \"%s\" &", ipc_state.start_time, file_path);
    } else {
        snprintf(cmd, sizeof(cmd), "/usr/bin/rkffplay -hide_banner -nostats -an -fs \"%s\" &", file_path);
    }
    printf("%s\n", cmd);
    ret = system(cmd);
    if (ret) {
...
void media_sync_ext(char *ext_pos)
{
    ...
    if (ipc_state.ffplay_is_quit == 0) {
        m = alloc_ipc_message((char*)SYNC_TO, ext_pos);
        if (m) {
           push_ipc_message(&ipc_state, m);
        }
    }
