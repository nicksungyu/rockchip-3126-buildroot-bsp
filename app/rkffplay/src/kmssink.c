...
configure_mode_setting (KMSSINK * self, VideoInfo * vinfo)
{
  ...
  rinfo.width = vinfo->width;
  rinfo.height = vinfo->height;
  rinfo.format = vinfo->format;

  for (i = 0; i < conn->count_modes; i++) {
    if (conn->modes[i].vdisplay == rinfo.height &&
        conn->modes[i].hdisplay == rinfo.width) {
      mode = &conn->modes[i];
      break;
    }
  }
  if (!mode) {
    ...
    rinfo.width = mode->hdisplay;
    rinfo.height = mode->vdisplay;
    printf("#WARN: No matching display mode found, use default mode(%dx%d)\n",
        rinfo.width, rinfo.height);
    //goto mode_failed;
  }

  kmsmem = kms_allocator_bo_alloc (self, &rinfo);
  if (!kmsmem)
    goto bo_failed;
  fb_id = kmsmem->fb_id;
...
open_failed:
  {
    printf("Could not open DRM module reason: %s (%d)\n", strerror(errno), errno);
    return false;
  }

resources_failed:
  {
    printf("drmModeGetResources failed,reason: %s (%d)\n", strerror (errno), errno);
    goto bail;
  }
...
plane_resources_failed:
  {
    printf("drmModeGetPlaneResources failed, reason: %s (%d)\n", strerror (errno), errno);
    goto bail;
  }
...
bool kms_sink_stop (KMSSINK *self)
{
  ...
  if (self->saved_crtc) {
    ...
    if (err)
      printf("Failed to restore previous CRTC mode: %s\n", strerror (errno));
...
pageflip_failed:
  {
    printf("drmModePageFlip failed: %s (%d)\n",strerror (errno), errno);
    return false;
  }
event_failed:
  {
    printf("drmHandleEvent failed: %s (%d)\n",strerror (errno), errno);
    return false;
  }
}
