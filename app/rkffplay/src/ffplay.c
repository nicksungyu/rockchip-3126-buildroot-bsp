...
/* RockChip fb and rga api */
#include "rk_fb/rk_fb.h"
#include <rga/RgaApi.h>
#else
#ifdef CONFIG_ROCKCHIP_DRM_DIRECTLY
#include "kmssink.h"
#include "rga/RgaApi.h"
...
/* SDL2 force display of video on overlay plane?
 * 1: enbale force display on overlay plane
 * 0: disable force display on overlay plane
 */
...
static int g_rockchip_rkfb_init_flag;

static void ffplay_rockchip_transparent_ui_win(void)
{
    struct win * ui_win;
...
static int ffplay_rockchip_rv1108_display(Frame *vp)
{
    int src_fd, src_fmt;
    int src_w, src_h, vir_w, vir_h;
    int dst_fd, dst_fmt;
    int dst_w, dst_h;
    int disp_width = 0, disp_height = 0;
    struct win* video_win = NULL;
    AVDRMFrameDescriptor *desc = NULL;
    int pix_size = 0, offset = 0;
    int out_device, rotate_angle;
    int ret;

    if (vp->frame->format != AV_PIX_FMT_DRM_PRIME) {
        av_log(NULL, AV_LOG_ERROR, "Software decoding is not supported on rv1108 platform!");
        return -1;
    } else {
        if (!g_rockchip_rkfb_init_flag) {
            printf("===== rk_fb_init =====\n");
            av_log(NULL, AV_LOG_DEBUG, "Librkfb: rkfb init...\n");
            rk_fb_init(FB_FORMAT_RGB_565);
            rk_fb_set_yuv_range(CSC_BT601F);
            ffplay_rockchip_transparent_ui_win();
            g_rockchip_rkfb_init_flag = 1;
        }

        out_device = rk_fb_get_out_device(&disp_width, &disp_height);
        if (out_device == -1) {
            av_log(NULL, AV_LOG_ERROR, "Librkfb: get out device failed!");
            return -1;
        }
        ...
        if (!video_win) {
            av_log(NULL, AV_LOG_ERROR, "Librkfb: get video win failed!");
            return -1;
        }

        desc = (AVDRMFrameDescriptor*)vp->frame->data[0];
        src_w = vp->width;
        src_h = vp->height;
        vir_w = FFALIGN(vp->width, 16);
        vir_h = FFALIGN(vp->height, 16);
        ...
        src.fd = src_fd;
        dst.fd = dst_fd;
        rga_set_rect(&src.rect, 0, 0, src_w, src_h, vir_w, vir_h, src_fmt);
        rga_set_rect(&dst.rect, 0, 0, dst_w, dst_h, dst_w, dst_h, dst_fmt);
        rga_set_rotation(&src, rotate_angle);
        ret = c_RkRgaBlit(&src, &dst, NULL);
        if (ret) {
            av_log(NULL, AV_LOG_ERROR, "Librkrga: c_RkRgaBlit failed!"
                "\n\tapi input param:"
                "\n\tsrcfd:%d, width:%d, height:%d, v-width:%d, v-height:%d, fmt:%d"
                "\n\tdstfd:%d, disp-width:%d, disp-height:%d, fmt:%d, rotate_angle:%d",
                src_fd, vp->width, vp->height, vir_w, vir_h, src_fmt,
                dst_fd, disp_width, disp_height, dst_fmt, rotate_angle);
            return -1;
        }
    }

    if (rk_fb_video_disp(video_win) == -1) {
        av_log(NULL, AV_LOG_ERROR, "Librkfb: display video window failed\n");
        return -1;
    }
            ...
            do {
                ...
                switch (d->avctx->codec_type) {
                    case AVMEDIA_TYPE_VIDEO:
                        ...
                        if (ret >= 0) {
                            if ((decoder_reorder_pts == -1) && (frame->best_effort_timestamp != AV_NOPTS_VALUE))
                                frame->pts = frame->best_effort_timestamp;
                            else if ((!decoder_reorder_pts) && (frame->pkt_dts != AV_NOPTS_VALUE))
                                frame->pts = frame->pkt_dts;
                            else if ((frame->pts == AV_NOPTS_VALUE) && (frame->pkt_dts != AV_NOPTS_VALUE))
                                frame->pts = frame->pkt_dts;
                        }
...
#if CONFIG_ROCKCHIP_DRM_DIRECTLY

static void drm_image_display(VideoState *is, AVFrame *frame)
{
    static int rk_rga_inited = 0;
    static int drm_inited = 0;
    static int x_pos, y_pos;
    static int count = 0;
    static KMSSINK kmssink;
    ...
    bo_t bo_dst;
    int ret = 0, dst_fd = -1;
    AVDRMFrameDescriptor* drm_fram_desc;
    KMSMEM *buffer;
    VideoInfo vinfo;

    vinfo.format = DRM_FORMAT_ARGB8888;
    if (screen_width && screen_height) {
        vinfo.width = screen_width;
        vinfo.height = screen_height;
    } else {
        vinfo.width = frame->linesize[0] ? frame->linesize[0] : frame->width;
        vinfo.height = frame->height;
    }

    if (drm_inited == 0) {
        if(!kms_sink_start(&kmssink, &vinfo)) {
            av_log(NULL, AV_LOG_FATAL, "Init drm failed!\n");
            return;
        }
        drm_inited = 1;
        av_log(NULL, AV_LOG_INFO, "#VideoInfo: fmt 0x%x, %d x %d, linesize %d %d %d\n",
            frame->format, vinfo.width, vinfo.height, frame->linesize[0],
        ...
        if (autorotate) {
            float wh_ratio0, wh_ratio1;

            x_pos = y_pos = 0;
            dstWidth = kmssink.hdisplay;
            dstHeight = kmssink.vdisplay;

            wh_ratio0 = (float)vinfo.width / vinfo.height;
            wh_ratio1 = (float)kmssink.hdisplay / kmssink.vdisplay;

            if (((wh_ratio0 > 1.0) && (wh_ratio1 < 1.0)) ||
                ((wh_ratio0 < 1.0) && (wh_ratio1 > 1.0))) {
                av_log(NULL, AV_LOG_INFO, "##The image will be rotated 90¡ã\n");
                rotate = HAL_TRANSFORM_ROT_90;
            }
        } else {
            if (vinfo.width >= vinfo.height) {
                dstWidth = kmssink.hdisplay;
                dstHeight = (int)(vinfo.height * ((float)kmssink.hdisplay / vinfo.width));
                x_pos = 0;
                y_pos = (kmssink.vdisplay - dstHeight) / 2;
            } else {
                dstWidth = (int)(vinfo.width * ((float)kmssink.vdisplay / vinfo.height));
                dstHeight = kmssink.vdisplay;
                x_pos = (kmssink.hdisplay - dstWidth) / 2;
                y_pos = 0;
            }
        }
        kms_sink_set_render_rectangle(&kmssink, x_pos, y_pos, dstWidth, dstHeight);
    }
    ...
    vinfo.width = dstWidth;
    vinfo.height = dstHeight;
    buffer = kms_sink_get_mem(&kmssink, &vinfo);
    ...
    kms_sink_show_frame(&kmssink, buffer);
}

#endif /* CONFIG_ROCKCHIP_DRM_DIRECTLY */
...
static void video_display(VideoState *is)
{
    ...
    vp = frame_queue_peek_last(&is->pictq);
    vp->uploaded = 1;
    ffplay_rockchip_rv1108_display(vp);

#elif CONFIG_ROCKCHIP_DRM_DIRECTLY
    Frame *vp;

    vp = frame_queue_peek_last(&is->pictq);
    vp->uploaded = 1;

    drm_image_display(is, vp->frame);
    return;

#else /* CONFIG_ROCKCHIP_RV1108 */
...
static const OptionDef options[] = {
    ...
    { "forceoverlay", OPT_BOOL | OPT_EXPERT, { &force_overlay }, "force display on overlay plane", "" },
    { NULL, },
};
    ...
    if (!strcmp(key, SYNC_TO)) {
        ...
        for (i = 0; i < strlen(value); i++) {
            if ((*(value + i) >= '0') && (*(value + i) <= '9'))
                new_pos = new_pos * 10 + (*(value + i) - '0');
            else
                break;
        }

        cur_pos = (int64_t)(get_master_clock(is) * 1000000.0);
        av_log(NULL, AV_LOG_INFO, "new_pos: %ld, cur_pos: %ld(%f), delta:%ld\n",
            new_pos, cur_pos, get_master_clock(is), new_pos - cur_pos);

        if (fabs(cur_pos - new_pos) > 200000) {

            tns  = cur_pos / 1000000LL;
            thh  = tns / 3600;
            tmm  = (tns % 3600) / 60;
            tss  = (tns % 60);

            ns   = new_pos / 1000000LL;
            hh   = ns / 3600;
            mm   = (ns % 3600) / 60;
            ss   = (ns % 60);

            av_log(NULL, AV_LOG_INFO, "Seek frome (%2d:%02d:%02d) to (%2d:%02d:%02d)\n",
                thh, tmm, tss, hh, mm, ss);

            if (is->ic->start_time != AV_NOPTS_VALUE)
                new_pos += is->ic->start_time;
                stream_seek(is, new_pos, 0, 0);
            }
        goto response;
    }
...
int main(int argc, char **argv)
{
    ...
    if (force_overlay) {
        if (setenv("SDL2_DISPLAY_PLANE_TYPE", "OVERLAY", 1)) {
            av_log(NULL, AV_LOG_FATAL, "Set SDL2_DISPLAY_PLANE_TYPE env failed!\n");
            push_ipc_state_message(&ipc_state, ERROR);
            deinit_ipc(&ipc_state);
            exit(1);
        }
...
#if defined CONFIG_ROCKCHIP_RV1108 || defined CONFIG_ROCKCHIP_DRM_DIRECTLY
...
#if CONFIG_ROCKCHIP_RV1108
    if (g_rockchip_rkfb_init_flag) {
        rk_fb_deinit();
        av_log(NULL, AV_LOG_DEBUG, "Librkfb: rkfb deinit...\n");
    }
